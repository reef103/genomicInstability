...
giDensityPlot <- function(inferCNV, legend=c("topleft", "top", "topright", "none"), ...) {
    ...
    if (!is.null(inferCNV[["gisnull"]])) {
        ...
        dnull$y <- dnull$y*max(density(inferCNV[["gis"]], na.rm=TRUE)$y)/max(dnull$y)
        lines(dnull, lwd=2)
        if (legend != "none")
            legend(legend, c("Samples", "Null model"), fill=c("grey", "black"), bty="n")
    }
}
...
plot.inferCNV <- function(inferCNV, output=NULL, threshold=.2, gamma=1.5, resolution=150) {
    ...
    checkmate::assertNumeric(threshold, lower=0, upper=1, any.missing=FALSE, len=1)
    checkmate::assertNumeric(gamma, lower=0, upper=10, any.missing=FALSE, len=1)
    checkmate::assertInt(resolution, lower=10, upper=1e4)
    # If threshold is between 0 and 1 and gi_likelihood is missing, compute gi_likelihood
    ...
    if (threshold > 0 & threshold < 1) {
        ...
        nes <- list()
        nes[["Unstable"]] <- t(inferCNV[["nes"]])[inferCNV[["gi_likelihood"]] >= threshold[2], , drop=FALSE]
        nes[["Intermediate"]] <- t(inferCNV[["nes"]])[inferCNV[["gi_likelihood"]] > threshold[1] & inferCNV[["gi_likelihood"]] < threshold[2], , drop=FALSE]
        if (threshold[1]==threshold[2]) {
            nes[["Stable"]] <- t(inferCNV[["nes"]])[inferCNV[["gi_likelihood"]] < threshold[1], , drop=FALSE]
        } else {
            nes[["Stable"]] <- t(inferCNV[["nes"]])[inferCNV[["gi_likelihood"]] <= threshold[1], , drop=FALSE]
        }
    ...
    widths <- chrom_size/100+.02
    widths[1] <- widths[1]+.4-.01
    widths[length(widths)] <- widths[length(widths)]+.1-.01
    heights <- group_size/200+.02
    heights[1] <- heights[1]+.4-.01
    heights[length(heights)] <- heights[length(heights)]+.1-.01
    # Generating the plot
    output_flag <- FALSE # Flag to whether the graphic device should be closed
    if (!is.null(output)) {
        if (length(grep("\\.pdf", output))>0) {
            pdf(output, w=sum(widths), h=sum(heights), pointsize=10, useD=FALSE)
            output_flag <- TRUE # Flag to close the device
        }
        else if (length(grep("\\.png", output))>0) {
            png(output, w=sum(widths), h=sum(heights), units="in", res=resolution)
            output_flag <- TRUE # Flag to close the device
        }
        else if (length(grep("\\.jpg", output))>0) {
            jpeg(output, w=sum(widths), h=sum(heights), units="in", res=resolution)
            output_flag <- TRUE # Flag to close the device
        }
    }
    layout(matrix(1:(length(chrom_size)*length(group_size)), length(group_size), length(chrom_size), byrow=TRUE), widths=widths, heights=heights)
    for (group in 1:length(group_size)) {
        for(chrom in 1:length(chrom_size)) {
            # Defining the margins
            if (group==1 & chrom==1) par(mai=c(.01, .4, .4, .01))
            else if (group==length(group_size) & chrom==1) par(mai=c(.1, .4, .01, .01))
            else if (chrom==1) par(mai=c(.01, .4, .01, .01))
            else if (group==1 & chrom==length(chrom_size)) par(mai=c(.01, .01, .4, .1))
            else if (group==length(group_size) & chrom==length(chrom_size)) par(mai=c(.1, .01, .01, .1))
            else if (chrom==length(chrom_size)) par(mai=c(.01, .01, .01, .1))
            else if (group==1) par(mai=c(.01, .01, .4, .01))
...
splitByChromosome <- function(x) {
    ...
    res <- tapply(1:ncol(x), chr, subsetMatrixByColumns, x=x)
    ...
    chr_name[pos] <- (1:length(pos))+length(which(chr_numeric))
...
plothm. <- function(x, color=c("royalblue","firebrick2"), gama=1, grid=T, scmax=0, box=TRUE, ...) {
    coli <- colorScale(x=x[nrow(x):1, , drop=FALSE], color=color, gama=gama, scmax=scmax)
...
transformNesForPlot <- function(x, method=c("scale", "equalize")) {
    ...
    transformation <- nesTransformationFunction(as.vector(unlist(x, use.names=FALSE)), method=method)
