# Functions required for the analysis and inference of genetic intability from RNA-Seq
...
#' @param skip Interger indicating the displacement of the window for selecting the k genes
#' 
...
#' 
#' @export
generateChromosomeGeneSet <- function(species=c("human", "mouse"), k=100, skip=25) {
    ...
    switch(species,
           human=data(hg38, package="genomicInstability", envir=environment()),
           mouse=data(mm10, package="genomicInstability", envir=environment())
    )
    ...
    pos <- unique(c(grep("_alt", names(genes_per_chromosome)), grep("_random", names(genes_per_chromosome))))
    ...
    geneset_chromosome <- lapply(genes_per_chromosome, splitVectorWindow, k=k, skip=skip)
    ...
    names(geneset) <- paste(rep(names(geneset_chromosome), sapply(geneset_chromosome, length)), unlist(lapply(geneset_chromosome, names), use.names=FALSE), sep="-")
    return(geneset)
}
...
#' 
#' @param expmat Matrix of gene expression profiles or signatures with genes (entrezID) in rows and samples in columns
#' @param nullmat Optional matrix with same number of rows as \code{expmat} to be used as null model
#' @param species Character string indicating the species, either human or mouse
#' @param k Integer indicating the number of genes per set
#' @param skip Interger indicating the displacement of the window for selecting the k genes
...
#' 
#' @return Object of class inferCNV, which is a list containing matrix of nes, and parameters (param), including species, window (k) and skip
#' 
#' @examples
...
#' 
#' @export
inferCNV <- function(expmat, nullmat=NULL, species=c("human", "mouse"), k=100, skip=25, min_geneset=10, verbose=TRUE) {
    # Check values for species
    species <- match.arg(species)
    # Validate input
    checkmate::assertMatrix(expmat, mode="numeric", all.missing=FALSE, min.rows=1000, min.cols=1, row.names="named", col.names="named")
    checkmate::assertMatrix(nullmat, mode="numeric", all.missing=FALSE, min.rows=1000, min.cols=1, row.names="named", null.ok=TRUE)
    checkmate::assertInt(k, lower=10, upper=1000)
    checkmate::assertInt(skip, lower=1, upper=k)
    ...
    if (!is.null(nullmat)) {
        ...
        if (length(genes) < 100) stop("Genes in expmat and nullmat do not match")
        expmat <- expmat[match(genes, rownames(expmat)), , drop=FALSE]
        nullmat <- nullmat[match(genes, rownames(nullmat)), , drop=FALSE]
    }
    # Generate genesets
    if (verbose) message("Generating the genesets from the genome information")
    ...
    if (length(geneset)==0) stop(paste0("No geneset with at least ", min_geneset, " genes"))
    # Enrichment for the expmat
    if (verbose) message("Computing the enrichment for the genesets in the expression matrix")
    expmat_nes <- sREA(expmat, geneset)
    # Enrichment of the null model
    nullnes <- NULL # Initialize the null nes variable
    if (!is.null(nullmat)) {
        if (verbose) message("Computing null model")
        expmat_null <- sREA(nullmat, geneset)
        # Estimating NES
        if (verbose) message("Estimating the normalized enrichment scores")
        expmat_nes <- t(sapply(1:nrow(expmat_nes), computeNesForMatrixRow, nesmat=expmat_nes, nullmat=expmat_null))
        nullnes <- t(sapply(1:nrow(expmat_null), computeNesForMatrixRow, nesmat=expmat_null, nullmat=expmat_null))
        rownames(expmat_nes) <- rownames(nullnes) <- names(geneset)
    }
    # Returning the results
    res <- list(nes=expmat_nes, null=nullnes, param=list(species=species, k=k, skip=skip))
    class(res) <- "inferCNV"
    return(res)
}
...
#' @param likelihood Logical, whether the genomic instability likelihood should be estimated
#' 
...
#' 
#' @seealso [inferCNV()] to infer the enrichment of loci-blocks in the gene expression data.
#' @export
...
#' @param distros Vector of 2 integers indicating the minimum and maximum number of Gaussian models to fit
#' @param tumor Optional vector of integers indicating the Gaussians considered as tumors
#' @param normal Optional vector of integers indicating the Gaussians considered as normal. This is only useful when no null model has been provided for the analysis
#' 
...
#' 
#' @seealso [genomicInstabilityScore()] to estimate the genomic instability score for
#' each cell in the dataset, and [inferCNV()] to infer the enrichment of loci-blocks
#' in the gene expression data.
#' @export
giLikelihood <- function(inferCNV, recompute=TRUE, distros=c(1, 3), tumor=NULL, normal=NULL) {
    # Validate inputs
    validateInferCNV(inferCNV, "nes")
    checkmate::assertLogical(recompute, len=1)
    checkmate::assertIntegerish(distros, lower=1, upper=10, len=2, any.missing=FALSE)
    checkmate::assertIntegerish(tumor, lower=1, upper=max(distros), any.missing=FALSE, null.ok=TRUE)
    checkmate::assertIntegerish(normal, lower=1, upper=max(distros), any.missing=FALSE, null.ok=TRUE)
    ...
    if (is.null(inferCNV[["gi_fit"]]) | recompute) {
        results_fit <- mixGaussianFit(inferCNV[["gis"]], min=distros[1], max=distros[2])
    ...
    if (any(tumor > length(results_fit[["mu"]])))
        stop("Tumor selection is larger than the number of models", call.=FALSE)
    ...
    if (!is.null(inferCNV[["gisnull"]]) & is.null(normal)) {
        ...
        results_fit[["mu"]] <- c(mean(inferCNV[["gisnull"]], na.rm=TRUE), results_fit[["mu"]][tumor])
        results_fit[["sigma"]] <- c(sd(inferCNV[["gisnull"]], na.rm=TRUE), results_fit[["sigma"]][tumor])
        # Adjust the tumor positions to account for the removed models and the null model
        tumor <- (1:length(tumor))+1
    } else {
        if (is.null(normal))
            normal <- 1
        if (any(normal > length(results_fit[["mu"]])))
            stop("Normal selection is larger than the number of models", call.=FALSE)
        ...
        tumor <- (1:length(tumor))+length(normal)
    }
    # Compute relative likelihood
    inferCNV[["gi_likelihood"]] <- rowSums(predict(results_fit, inferCNV[["gis"]], tumor))
    return(inferCNV)
}
