...
#' @param method Method to compute the genomic instability score, either "var" or
#' "meansq"
#' @param quantile Optional number between 0 and 1 indicating the quantile to
#' calculate GI-scores including only top / "hotspot" genes according to their
#' squared inferred CNV values across all cells.
...
genomicInstabilityScore <- function(cnv, method = c("var", "meansq"),
    quantile = NULL, likelihood = FALSE) {
    # Validating inputs
    ...
    meansq = {
        ...
        if (!is.null(cnv[["null"]])) {
            if (all(colnames(cnv[["null"]]) %in% colnames(cnv[["nes"]]))) {
                gisnull <- gis[match(colnames(cnv[["null"]]), colnames(cnv[["nes"]]))]
            } else {
                gisnull <- msq(cnv[["null"]], quantile = quantile)
            }
        }
    })
    cnv[["gis"]] <- gis
...
#' Mean squared method for genomic instability score
#'
#' This function computes the mean squared method for genomic instability score
#'
#' @param nes Matrix of inferred CNV scores
#' @param quantile Optional number between 0 and 1 indicating the quantile to
#' calculate GI-scores including only top / "hotspot" genes according to their
#' squared inferred CNV values across all cells.
#' @param ref Optional character vector of cell names to be used as reference for
#' the selection of the top genes. If NULL, all cells will be used for the selection of the top genes.
#' @return Numeric vector with the genomic instability score for each cell based on the mean squared method
#' @export
msq <- function(nes, quantile = NULL, ref = NULL) {
    checkmate::assertMatrix(nes, mode = "numeric", all.missing = FALSE, min.rows = 1,
...
#' This function selects hotspot genes based on mean squared CNA values
#' @param nes Matrix of inferred CNV scores
#' @param quantile Numeric value between 0 and 1 indicating the quantile to
#' calculate GI-scores including only top / "hotspot" genes according to their
#' squared inferred CNV values across all cells.
#' @return Character vector of gene names corresponding to the selected hotspot genes
#' @export
hotSpotGenes <- function(nes, quantile) {
    checkmate::assertMatrix(nes, mode = "numeric", all.missing = FALSE, min.rows = 1,
        min.cols = 1, row.names = "named", col.names = "named")
    checkmate::assertNumber(quantile, lower = 0, upper = 1, null.ok = TRUE)
    if (is.null(quantile)) {
    ...
    sqmat <- nes^2
    gene_scores <- rowMeans(sqmat, na.rm = TRUE)
    threshold <- quantile(gene_scores, probs = quantile, na.rm = TRUE)
    genes <- names(gene_scores)[gene_scores >= threshold]
    return(genes)
}
